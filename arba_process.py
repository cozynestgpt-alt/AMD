...
def make_arba_xlsx(results: list, ym: str, out_dir: Path) -> Path:
    ...
    # ── 구분 분류 함수 ──────────────────────────────────────
    def get_category(store: str) -> str:
        """매핑된 매장명(DEPT_TO_STORE 변환 후) → 구분(백화점/직영점/미입점행사)"""
        s = (store or "").strip()
        if s.endswith("(직영점)"):
            return "직영점"
        if s.endswith("(미입점행사)"):
    ...
    # ── 보험료 집계 테이블 ─────────────────────────────────
    # 구분별로 보험료 합산
    # 전체목록 시트에서 매장명(C열)·고용보험(J열)·의료보험(K열)·국민연금(L열)·실지급액(N열)·총지급액(I열) 읽기
    ws_all = wb["전체목록"]
    ...
    for dr in range(5, ws_all.max_row + 1):
        ...
        cat_v    = get_category(str(store_v))
