...
- **2026-08-07 재고실사 매장명 매칭 누락 버그 수정 (PR #59)**: `expense_report.py`의
  `load_inventory()`가 재고실사 파일의 매장명을 `INV_NAME_MAP`(5월분 파일 기준으로
  하드코딩해둔 약칭→정식명 매핑표)으로만 변환하고 있어서, 07월 파일에 새로 등장한
  "점" 자만 빠진 약칭 5개(롯데평촌/신세계센텀/신세계아울렛파주/현대아울렛남양주/
  롯데포항)가 매핑 실패로 조용히 사라지는 버그가 있었음. `경비지원및공제_집계.xlsx`
  E열(유통하자/재고LOSS) 합계가 입력 파일 원본 695,550원이어야 할 것이 235,500원으로
  나와 460,050원이 누락된 것을 발견해 원인 확정 후 수정. 매달 이런 "점 자만 빠진
  새 약칭"이 반복될 걸로 보여, 매번 `INV_NAME_MAP`에 항목을 추가하지 않아도 되도록
  "정식명 직접일치 → INV_NAME_MAP → 원본+점 보정" 순 일반 폴백을 추가했고, 그래도
  매칭 안 되면 콘솔에 경고를 출력해 향후 동일 유형 누락이 조용히 묻히지 않게 함.
  07월 전체 파이프라인(`run.py`)을 로컬에서 재실행해 E열 합계가 -695,550원(입력
  파일 원본과 일치)으로 정상 확인됨.
