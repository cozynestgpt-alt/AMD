...
## 의도적 지표 재정의 (버그 아님 — 값이 예전과 달라도 정상)
`AMD_backup_20260701` 백업본과 비교했을 때 계산식 자체가 바뀐 지표들. 리포트 값이
백업본 수치와 다르게 나와도 되돌리거나 "버그 수정"하지 말 것 — 경영진이 보고받는
방식을 바꾸기로 한 의도적 결정이다 (2026-07-21 사용자 확인).
- `year_compare_report.py` 동일매장분석 시트 "공헌이익률": 이전엔 "이 매장의
  영업이익 증감액 / 전체 매장 영업이익 증감액 합계"(전년대비 이익 변화 기여도)였는데,
  지금은 "이 매장의 당해년도 영업이익 / 동일매장 전체 당해년도 영업이익 합계"
  (전체 이익 중 비중)로 바뀜.
- `sales_analysis.py` 매출집계 시트 "순이익율"/"경비율": 이전엔 매출(V-,
  판매금액/1.1) 기준이었는데, 지금은 영업이익(V-) 기준으로 바뀜.
- 앞으로 이 두 파일에서 비슷한 계산식 변경(분모/분자 재정의)을 더 발견하면,
  버그로 단정하기 전에 먼저 의도된 변경인지 확인할 것.

## 백업본 폴더 참고
- `AMD_backup_20260701`: 2026-06-30 시점(정식 Git 저장소로 재구성되기 직전)의
  스냅샷 백업. 스크립트 + input/output 데이터만 있는 단순 작업 폴더 형태로,
  git 이력이 없다. **현재 운영 코드가 아니다** — 절대 이 폴더를 수정하거나 이
  폴더 기준으로 배포하지 말 것. 예전 계산 로직과 비교(diff)할 때만 참고 용도로
  쓴다.
...
- **"매출은 있는데 사원 미배정" 경고 — 정상 케이스 2곳**: 위 PR #48/#49로
  추가된 이 경고가 `현대압구정본점`, `롯데아울렛서울역점`에 대해 뜨는 건
  문제가 아니다. 두 매장은 본사 직원 파견 형태로 운영되어 원래부터
  사원마스터에 담당자가 등록되지 않는다(2026-07-15 확인). 이 두 매장
  **이외의** 매장에서 이 경고가 뜨면 그건 실제 사원마스터 입력 누락일
  가능성이 높으니 실무자 확인이 필요하다.
