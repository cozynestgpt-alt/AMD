...
## 매뉴얼 문서 보관 원칙
- 관리자용(Git/PR/브랜치 보호 등 개발 워크플로우) 매뉴얼은 GitHub 저장소 `docs/`에,
  팀원용 실행 매뉴얼(입력파일 준비, 배치파일 사용법 등)은 NAS `docs/`에 보관한다.
- NAS(`salary_system_claude`) `docs/` 폴더 목록 (2026-07-10 기준, 대부분 V4~V8 개발
  이력 중 운영기준으로 남겨둔 문서들이고 GitHub `docs/`에도 동일 파일이 있음 —
  Git/PR 워크플로우 매뉴얼 2건만 예외로 GitHub에만 있고 여기엔 없음):
